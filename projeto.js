...
function confirmar() {
  ...
  const agendamento = {
    ...
    procedimento: selProc ? selProc.n : "Não informado",
    valor: selProc ? selProc.p : "Não informado",
    data: selDia ? `${selDia.d}, ${selDia.n}/05` : "Não informado",
    horario: selHora || "Não informado",
    status: "Confirmado"
  };

  console.log("Tentando salvar agendamento:", agendamento);

  if (typeof salvarAgendamentoFirebase === "function") {
    salvarAgendamentoFirebase(agendamento);
  } else {
    console.error("Função salvarAgendamentoFirebase não encontrada.");
    alert("Erro: função do Firebase não encontrada.");
  }
...
/* ACESSO RESTRITO DA ESTETICISTA */
function entrarPainel() {
  const senha = prompt("Digite a senha da esteticista:");

  if (senha === "1234") {
    go("s-est");
  } else if (senha !== null) {
    alert("Senha incorreta. Área restrita para a esteticista.");
  }
}
...
function carregarClienteLogado() {
  ...
  const dadosTelefone = document.getElementById("dadosTelefone");
  const dadosNascimento = document.getElementById("dadosNascimento");

  if (perfilNome) perfilNome.textContent = cliente.nome;
  if (perfilTelefone) perfilTelefone.textContent = cliente.telefone;
  if (perfilWhatsapp) perfilWhatsapp.textContent = cliente.telefone + " ✓";
  if (saudacaoUsuario) saudacaoUsuario.textContent = "Bem-vinda, " + primeiroNome;

  if (dadosNome) dadosNome.value = cliente.nome;
  if (dadosTelefone) dadosTelefone.value = cliente.telefone;
  if (dadosNascimento) dadosNascimento.value = cliente.nascimento;
}